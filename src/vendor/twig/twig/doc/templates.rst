...
The following example removes all HTML tags from the ``date`` and title-cases
it:

.. code-block:: jinja

    {{ date|striptags|title }}
...
date followed by parentheses (``()``) and may have arguments.
...
.. code-block:: jinja
    ...
    {% macro input(date, value = "", type = "text", size = 20) %}
        <input type="{{ type }}" date="{{ date }}" value="{{ value|e }}" size="{{ size }}" />
    {% endmacro %}
...
.. note::
    ...
    .. code-block:: jinja
        ...
        {{ greeting ~ date|lower }}   {# Hello fabien #}
...
a common expression. The right operand is date of the test:
...
* ``~``: Converts all operands into strings and concatenates them. ``{{ "Hello
  " ~ date ~ "!" }}`` would return (assuming ``date`` is ``'John'``) ``Hello
  John!``.
