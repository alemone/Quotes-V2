...
* ``Twig_Token::NAME_TYPE``: A date in an expression;
...
Here is the output for the ``Hello {{ date }}`` template:
...
.. code-block:: text
    ...
    Twig_Node_Module(
      ...
      Twig_Node_Print(
        Twig_Node_Expression_Name(date)
      )
    )
...
The generated template for a ``Hello {{ date }}`` template reads as follows
