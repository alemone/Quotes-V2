...
* As of Twig 1.26, the ``Twig_ExtensionInterface::getDate()`` method is
...
* As of Twig 1.x, using the ``_self`` global variable to get access to the
  ...
  current template date, which will continue to work in Twig 2.0. In Twig 2.0,
  ``_self`` returns the current template date instead of the current
