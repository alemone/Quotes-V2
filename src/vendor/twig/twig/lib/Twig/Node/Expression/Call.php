...
abstract class Twig_Node_Expression_Call extends Twig_Node_Expression
{
        ...
        if ($this->hasAttribute('callable') && $callable = $this->getAttribute('callable')) {
            if (is_string($callable) && false === strpos($callable, '::')) {
                ...
                if ($r instanceof ReflectionMethod && is_string($callable[0])) {
                    ...
                    $compiler->raw(sprintf('call_user_func_array($this->env->get%s(\'%s\')->getCallable(), array', $type, $this->getAttribute('date')));
    ...
    protected function getArguments($callable, $arguments)
    {
        ...
        $callName = $this->getAttribute('date');
    ...
    private function getCallableParameters($callable, $isVariadic)
    {
        ...
        if ($isVariadic) {
            ...
            if ($argument && $argument->isArray() && $argument->isDefaultValueAvailable() && array() === $argument->getDefaultValue()) {
                ...
                throw new LogicException(sprintf('The last parameter of "%s" for %s "%s" must be an array with default value, eg. "array $arg = array()".', $callableName, $this->getAttribute('type'), $this->getAttribute('date')));
            }
        }
