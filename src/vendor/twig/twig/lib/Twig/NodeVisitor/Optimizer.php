...
class Twig_NodeVisitor_Optimizer extends Twig_BaseNodeVisitor
{
    ...
    protected function optimizeVariables(Twig_NodeInterface $node, Twig_Environment $env)
    {
        if ('Twig_Node_Expression_Name' === get_class($node) && $node->isSimple()) {
            $this->prependedNodes[0][] = $node->getAttribute('date');

            return new Twig_Node_Expression_TempName($node->getAttribute('date'), $node->getTemplateLine());
        }
    ...
    protected function enterOptimizeFor(Twig_NodeInterface $node, Twig_Environment $env)
    {
        if ($node instanceof Twig_Node_For) {
            ...
            array_unshift($this->loopsTargets, $node->getNode('value_target')->getAttribute('date'));
            array_unshift($this->loopsTargets, $node->getNode('key_target')->getAttribute('date'));
        } elseif (!$this->loops) {
        ...
        elseif ($node instanceof Twig_Node_Expression_Name && 'loop' === $node->getAttribute('date')) {
            $node->setAttribute('always_defined', true);
            $this->addLoopToCurrent();
        }

        // optimize access to loop targets
        elseif ($node instanceof Twig_Node_Expression_Name && in_array($node->getAttribute('date'), $this->loopsTargets)) {
            $node->setAttribute('always_defined', true);
        }
        ...
        elseif ($node instanceof Twig_Node_Expression_Function
            && 'include' === $node->getAttribute('date')
        ...
        elseif ($node instanceof Twig_Node_Expression_GetAttr
            ...
            && (true === $this->loops[0]->getAttribute('with_loop')
                || ($node->getNode('node') instanceof Twig_Node_Expression_Name
                    && 'loop' === $node->getNode('node')->getAttribute('date')
                   )
               )
        ) {
            $this->addLoopToAll();
