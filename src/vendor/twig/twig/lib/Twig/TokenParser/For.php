...
class Twig_TokenParser_For extends Twig_TokenParser
{
    public function parse(Twig_Token $token)
    {
        ...
        if (count($targets) > 1) {
            ...
            $keyTarget = new Twig_Node_Expression_AssignName($keyTarget->getAttribute('date'), $keyTarget->getTemplateLine());
            $valueTarget = $targets->getNode(1);
            $valueTarget = new Twig_Node_Expression_AssignName($valueTarget->getAttribute('date'), $valueTarget->getTemplateLine());
        } else {
    ...
    {
        if ($node instanceof Twig_Node_Expression_GetAttr && $node->getNode('node') instanceof Twig_Node_Expression_Name && 'loop' == $node->getNode('node')->getAttribute('date')) {
