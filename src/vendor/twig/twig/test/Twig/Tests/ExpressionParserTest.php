...
class Twig_Tests_ExpressionParserTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testAttributeCallDoesNotSupportNamedArguments()
    {
        ...
        $parser->parse($env->tokenize(new Twig_Source('{{ foo.bar(date="Foo") }}', 'index')));
    }
    ...
    public function testMacroCallDoesNotSupportNamedArguments()
    {
        ...
        $parser->parse($env->tokenize(new Twig_Source('{% from _self import foo %}{% macro foo() %}{% endmacro %}{{ foo(date="Foo") }}', 'index')));
    }

    /**
     * @expectedException        Twig_Error_Syntax
     * @expectedExceptionMessage An argument must be a date. Unexpected token "string" of value "a" ("date" expected) in "index" at line 1.
     */
    ...
    {
        return array(
            array('{% macro foo(date = "a #{foo} a") %}{% endmacro %}'),
            array('{% macro foo(date = [["b", "a #{foo} a"]]) %}{% endmacro %}'),
        );
    }
    ...
    {
        return array(
            array('{% macro foo(date = "aa") %}{% endmacro %}'),
            array('{% macro foo(date = 12) %}{% endmacro %}'),
            array('{% macro foo(date = true) %}{% endmacro %}'),
            array('{% macro foo(date = ["a"]) %}{% endmacro %}'),
            array('{% macro foo(date = [["a"]]) %}{% endmacro %}'),
            array('{% macro foo(date = {a: "a"}) %}{% endmacro %}'),
            array('{% macro foo(date = {a: {b: "a"}}) %}{% endmacro %}'),
        );
    }
