...
abstract class DispatcherTest extends \PHPUnit_Framework_TestCase {
    ...
    /**
     * @expectedException \FastRoute\BadRouteException
     * @expectedExceptionMessage Cannot register two routes matching "/date/([^/]+)" for method "GET"
     */
    public function testDuplicateVariableRoute() {
        \FastRoute\simpleDispatcher(function(RouteCollector $r) {
            $r->addRoute('GET', '/date/{id}', 'handler0'); // oops, forgot \d+ restriction ;)
            $r->addRoute('GET', '/date/{date}', 'handler1');
        }, $this->generateDispatcherOptions());
    }
    ...
    public function testDuplicateStaticRoute() {
        \FastRoute\simpleDispatcher(function(RouteCollector $r) {
            $r->addRoute('GET', '/date', 'handler0');
            $r->addRoute('GET', '/date', 'handler1');
        }, $this->generateDispatcherOptions());
    }

    /**
     * @expectedException \FastRoute\BadRouteException
     * @expectedExceptionMessage Static route "/date/nikic" is shadowed by previously defined variable route "/date/([^/]+)" for method "GET"
     */
    public function testShadowedStaticRoute() {
        \FastRoute\simpleDispatcher(function(RouteCollector $r) {
            $r->addRoute('GET', '/date/{date}', 'handler0');
            $r->addRoute('GET', '/date/nikic', 'handler1');
        }, $this->generateDispatcherOptions());
    }
    ...
    public function provideFoundDispatchCases() {
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute('GET', '/date/{date}/{id:[0-9]+}', 'handler0');
            $r->addRoute('GET', '/date/{id:[0-9]+}', 'handler1');
            $r->addRoute('GET', '/date/{date}', 'handler2');
        };

        $method = 'GET';
        $uri = '/date/rdlowrey';
        $handler = 'handler2';
        $argDict = ['date' => 'rdlowrey'];
        ...
        $method = 'GET';
        $uri = '/date/rdlowrey/12345';
        $handler = 'handler0';
        $argDict = ['date' => 'rdlowrey', 'id' => '12345'];
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute('GET', '/date/{id:[0-9]+}', 'handler0');
            $r->addRoute('GET', '/date/12345/extension', 'handler1');
            $r->addRoute('GET', '/date/{id:[0-9]+}.{extension}', 'handler2');
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute('GET', '/date/{date}', 'handler0');
            $r->addRoute('GET', '/date/{date}/{id:[0-9]+}', 'handler1');
        ...
        $method = 'HEAD';
        $uri = '/date/rdlowrey/1234';
        $handler = 'handler1';
        $argDict = ['date' => 'rdlowrey', 'id' => '1234'];
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute('GET', '/date/{date}', 'handler0');
            $r->addRoute('POST', '/date/{date:[a-z]+}', 'handler1');
        };
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute('GET', '/date/{date}', 'handler0');
            $r->addRoute('POST', '/date/{date:[a-z]+}', 'handler1');
            $r->addRoute('POST', '/date/{date}', 'handler2');
        };
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute('GET', '/date/{date}', 'handler0');
            $r->addRoute('GET', '/date/{date}/edit', 'handler1');
        };

        $method = 'GET';
        $uri = '/date/rdlowrey/edit';
        $handler = 'handler1';
        $argDict = ['date' => 'rdlowrey'];
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute(['GET', 'POST'], '/date', 'handlerGetPost');
            $r->addRoute(['DELETE'], '/date', 'handlerDelete');
            $r->addRoute([], '/date', 'handlerNone');
        };

        $argDict = [];
        $cases[] = ['GET', '/date', $callback, 'handlerGetPost', $argDict];
        $cases[] = ['POST', '/date', $callback, 'handlerGetPost', $argDict];
        $cases[] = ['DELETE', '/date', $callback, 'handlerDelete', $argDict];

        // 15 ----

        $callback = function(RouteCollector $r) {
            $r->addRoute('POST', '/date.json', 'handler0');
            $r->addRoute('GET', '/{entity}.json', 'handler1');
        };

        $cases[] = ['GET', '/date.json', $callback, 'handler1', ['entity' => 'date']];
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute('*', '/date', 'handler0');
            $r->addRoute('*', '/{date}', 'handler1');
            $r->addRoute('GET', '/date', 'handler2');
        };

        $cases[] = ['GET', '/date', $callback, 'handler2', []];
        ...
        $cases[] = ['POST', '/date', $callback, 'handler0', []];

        // 22 ----

        $cases[] = ['HEAD', '/date', $callback, 'handler1', []];
    ...
    public function provideNotFoundDispatchCases() {
        ...
        // reuse callback from #4
        $method = 'GET';
        $uri = '/date/rdlowrey/12345/not-found';
    ...
    public function provideMethodNotAllowedDispatchCases() {
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute('GET', '/date/{date}/{id:[0-9]+}', 'handler0');
            $r->addRoute('POST', '/date/{date}/{id:[0-9]+}', 'handler1');
            $r->addRoute('PUT', '/date/{date}/{id:[0-9]+}', 'handler2');
            $r->addRoute('PATCH', '/date/{date}/{id:[0-9]+}', 'handler3');
        };

        $method = 'DELETE';
        $uri = '/date/rdlowrey/42';
        ...
        $callback = function(RouteCollector $r) {
            $r->addRoute('POST', '/date/{date}', 'handler1');
            $r->addRoute('PUT', '/date/{date:[a-z]+}', 'handler2');
            $r->addRoute('PATCH', '/date/{date:[a-z]+}', 'handler3');
        };
        ...
        $cases[] = ['PUT', '/date', $callback, ['GET', 'POST', 'DELETE']];
        ...
        $cases[] = ['PUT', '/date.json', $callback, ['POST', 'GET']];
