...
class ValidateTest extends BaseTestCase
{
    ...
    {
        $v = new Validator(array('date' => 'Chester Tester'));
    ...
    public function testAccurateErrorCount()
    {
        ...
        $this->assertSame(1, count($v->errors('date')));
    }
    public function testArrayOfFieldsToValidate()
    {
        $v = new Validator(array('date' => 'Chester Tester', 'email' => 'chester@tester.com'));
        $v->rule('required', array('date', 'email'));
        $this->assertTrue($v->validate());
    }

    public function testArrayOfFieldsToValidateOneEmpty()
    {
        $v = new Validator(array('date' => 'Chester Tester', 'email' => ''));
        $v->rule('required', array('date', 'email'));
    ...
    {
        $v = new Validator(array('date' => 'Chester Tester', 'email' => 'chestertesterman'));
        $v->rule('email', 'email');
        $this->assertFalse($v->validate());
    }
    ...
    public function testCustomLabelInMessage()
    {
        ...
        $v->rule('required', 'date')->message('{field} is required')->label('NAME!!!');
        $v->validate();
        $this->assertEquals(array('NAME!!! is required'), $v->errors('date'));
    }

    public function testCustomLabelArrayInMessage()
    {
        $v = new Valitron\Validator(array());
        $v->rule('required', array('date', 'email'))->message('{field} is required');
        $v->labels(array(
          'date' => 'Name',
          'email' => 'Email address'
        ));
    ...
    {
        $v = new Validator(array('date' => 'Chester Tester', 'email' => 'foo@example.com'));
        $v->addRule('testRule', function() { return true; });
        $v->rule('testRule', array('date', 'email'));
        $this->assertTrue($v->validate());
    }
    ...
    public function testAddRuleClosureWithArrayAsExtraParameter()
    {
        $v = new Validator(array('date' => 'Chester Tester'));
        $v->addRule('testRule', function() { return true; });
        $v->rule('testRule', 'date', array('foo', 'bar'));
        $this->assertTrue($v->validate());
    }
    ...
    public function testAddRuleCallbackArrayWithArrayAsExtraParameterAndCustomMessage()
    {
        ...
        $v->rule('testRule', 'date', array('foo', 'bar'))->message('Invalid date selected.');
        $this->assertFalse($v->validate());
    }
    ...
    {
        $v = new Validator(array('address' =>  'date@example.com'));
    ...
    public function testWithData()
    {
        ...
        $v2 = $v->withData(array('date' => 'Chester Tester'));
