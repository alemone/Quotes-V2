...
```php
$v = new Valitron\Validator(array('date' => 'Chester Tester'));
...
date, a custom callback or closure, and a error message to display in
...
```php
$v = new Valitron\Validator(array());
$v->rule('required', array('date', 'email'))->message('{field} is required')->label('Name');
$v->validate();
```
