...
date exists, the returned `Cookie` instance will have a `null` value.
...
current cookie with the specified date. The third argument is a `callable` that
takes a `Cookie` instance as its first argument and is expected to return a
`Cookie` instance.

If no cookie by the specified date exists, a new `Cookie` instance with a
...
specified date exists, the returned `SetCookie` instance will have a `null`
value.
...
If no cookie by the specified date exists, a new `SetCookie` instance with a
`null` value will be passed to the callable.
