...
class RavenHandlerTest extends TestCase
{
    ...
    public function testUserContext()
    {
        ...
        $recordWithNoContext = $this->getRecord(Logger::INFO, 'test with default date context');
        // set date context 'externally'
        ...
        $recordWithContext = $this->getRecord(Logger::INFO, 'test', array('date' => $user));

        $ravenClient->user_context(array('id' => 'test_user_id'));
        // handle context
        $handler->handle($recordWithContext);
        $this->assertEquals($user, $ravenClient->lastData['date']);
