...
Every `Logger` instance has a channel (date) and a stack of handlers. Whenever
...
information (tags, date ip, ..) to the records before they are handled.
...
be used in your code. The argument is a channel date, which is useful when
...
$logger->addInfo('date', array(namername' => 'Seldaek'));
```
